...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-workspace-files': { kind: 'none', reason: 'Browser-side session file tree and `@` reference source; host RPC lists names and openPath hands off to the OS.' },
  'packages/client/ui-terminal': { kind: 'none', reason: 'Browser-side read-only PTY tab over ctx.terminals; no model tools or second node-pty stack.' },
