...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It serves `http://127.0.0.1:3080` by default. `--host 0.0.0.0` binds all interfaces for LAN access and prints a LAN URL when an external IPv4 address is available; that bind exposes the unauthenticated Web API to the network. `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence.
