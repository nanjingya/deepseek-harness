...
生产 Web 运行器需要已构建的包和前端产物（`pnpm run build`）。默认服务地址是 `http://127.0.0.1:3080`。`--host 0.0.0.0` 绑定全部网卡供局域网访问，并在有外部 IPv4 时打印 LAN URL；该绑定会把未认证的 Web API 暴露给该网络。`--trusted-host` 可添加 `/api` 浏览器信任围栏接受的具名 authority。
